...
module.exports = function (userCfgPath) {
	const convictSchema = {
		...
		trueHost: {
			doc: 'The Nginx IP address or hostname that NodeCG should bind to.',
			format: String,
			default: ''
		},
		port: {
	...
	if(config.trueHost == ''){
		...
		config.baseURL = config.baseURL || `${config.host === '0.0.0.0' ? 'localhost' : config.trueHost}:${config.port}`;
	}
